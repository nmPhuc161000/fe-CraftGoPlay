...
const ManageProduct = () => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (!response?.success) {
          ...
          setError(
            response?.error ||
              "Lỗi kết nối API. Vui lòng kiểm tra cấu hình server."
          );
          setData([]);
          return;
        }
  ...
  const filtered = data.filter((p) => {
    ...
    const artisanMatch =
      p?.artisan?.toLowerCase()?.includes(searchLower) || false;
    const categoryMatch =
  ...
  const handleStatusChange = async () => {
    if (statusProduct?.id) {
      ...
      const updatedStatus =
        statusProduct.status === "active" ? "inactive" : "active";
      ...
      <div className="overflow-x-auto rounded-xl shadow">
        ...
        >
          <thead className="bg-gradient-to-r from-[#8b5e3c] to-[#c7903f]">
            <tr>
              <th className="px-2 py-3 text-center text-xs font-medium text-white uppercase tracking-wider w-12">
                STT
              </th>
              <th className="px-2 py-3 text-center text-xs font-medium text-white uppercase tracking-wider w-20">
                Hình ảnh
              </th>
              <th className="px-2 py-3 text-left text-xs font-medium text-white uppercase tracking-wider w-40">
                Tên sản phẩm
              </th>
              <th className="px-2 py-3 text-left text-xs font-medium text-white uppercase tracking-wider w-40">
                Thợ thủ công
              </th>
              <th className="px-2 py-3 text-left text-xs font-medium text-white uppercase tracking-wider w-32">
                Danh mục
              </th>
              <th className="px-2 py-3 text-center text-xs font-medium text-white uppercase tracking-wider w-24">
                Giá
              </th>
              <th className="px-2 py-3 text-center text-xs font-medium text-white uppercase tracking-wider w-32">
                Trạng thái
              </th>
              <th className="px-2 py-3 text-center text-xs font-medium text-white uppercase tracking-wider w-32">
                Số đơn hàng
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {paged.map((row, idx) => (
              ...
              >
                <td className="px-2 py-3 text-center overflow-hidden text-ellipsis w-12">
                ...
                <td
                  className="px-2 py-3 overflow-hidden text-ellipsis w-40"
                  title={row?.name || "N/A"}
                >
                ...
                <td
                  className="px-2 py-3 overflow-hidden text-ellipsis w-40"
                  title={row?.artisan || "N/A"}
                >
                  <div className="font-medium">{row?.artisan || "N/A"}</div>
                </td>
                <td
                  className="px-2 py-3 overflow-hidden text-ellipsis w-32"
                  title={row?.category || "N/A"}
                >
                  {row?.category || "N/A"}
                </td>
                <td className="px-2 py-3 text-center text-blue-700 font-bold w-24">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-gradient-to-r from-purple-50 to-purple-100 p-6 rounded-xl">
                      <div className="text-sm text-purple-600 font-medium mb-1">
                        Thông tin thợ thủ công
                      </div>
                      <div className="text-xl font-bold text-gray-900">
                        {viewProduct?.artisan || "N/A"}
                      </div>
                    </div>
