...
const ArtisanAccount = () => {
  ...
  // Fetch data from adminService.getAllAccount
  useEffect(() => {
    const fetchAccounts = async () => {
      try {
        const response = await adminService.getAllAccount({
          ...
          role: "Artisan", // Default role set to Artisan
        });
  ...
  // Handle clicks outside action menu to close it
  useEffect(() => {
  ...
  // Placeholder for handleActive and handleDeactivate
  ...
  const handleConfirm = () => {
    // Implement actual activation/deactivation logic here
    console.log(`Confirm ${confirmModal.action} for index ${confirmModal.idx}`);
        ...
        <div className="space-y-6 max-w-full">
          <div className="bg-amber-25 rounded-xl shadow p-4 overflow-x-auto">
            ...
            <div className="flex items-center justify-between mt-2 text-xs text-gray-500">
              <span>
                {filteredData.length > 0 ? (pageIndex - 1) * pageSize + 1 : 0} to{" "}
                {Math.min(pageIndex * pageSize, filteredData.length)} of {data.length}
              </span>
