    ...
    <>
      ...
      >
        ...
        <nav className="flex flex-col gap-2 px-4 py-6 flex-1 overflow-y-auto">
          {/* Dashboard */}
          <button
            className={`group flex items-center gap-4 w-full px-4 py-3 rounded-xl transition-all duration-200 text-base font-semibold relative overflow-hidden ${selected === 'dashboard'
          ...
          >
            <button
              ref={managerBtnRef}
              className={`group flex items-center gap-4 w-full px-4 py-3 rounded-xl transition-all duration-200 text-base font-semibold relative overflow-hidden ${managerSelectedList.includes(selected)
            ...
            {isDesktopCollapsed && hoverManager && (
              ...
              >
                ...
                {managerAccountSubMenu.map((item, index) => (
                  <button
                    key={item.value}
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-gradient-to-r text-left text-base font-medium whitespace-nowrap transition-all duration-200 hover:scale-102 ${selected === item.value
              ...
              <div className={`overflow-hidden transition-all duration-300 ease-in-out ${openManager ? 'max-h-96 opacity-100 mt-2' : 'max-h-0 opacity-0'
                ...
                <div className="pl-8 flex flex-col gap-1 py-2">
                  {managerAccountSubMenu.map((item, index) => (
                    <button
                      key={item.value}
                      className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-left text-base font-medium transition-all duration-200 hover:scale-102 ${selected === item.value
                    ...
                    >
                      <div className={`w-2 h-2 rounded-full transition-all duration-200 ${selected === item.value ? 'bg-purple-500' : 'bg-slate-400'
                        }`}></div>
                      <span>{item.label}</span>
                    </button>
                  ))}
