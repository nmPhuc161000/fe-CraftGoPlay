                ...
                if (res?.data?.error === 0 && Array.isArray(res?.data?.data)) {
                    ...
                    if (status === "") {
                        ...
                        const counts = statusFilters.reduce((acc, filter) => {
                            ...
                            else acc[filter.value] = data.filter((r) => r.status === filter.value).length;
                            return acc;
                        }, {});
                    ...
                    {statusFilters.map((filter) => {
                        ...
                        return (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200 whitespace-nowrap flex items-center gap-2 ${active
                            ...
                            >
                                {filter.label}
                                <span
                                    className={`text-xs px-2 py-0.5 rounded-full ${active ? "bg-white text-[#5e3a1e]" : "bg-gray-100 text-gray-600"
                                        }`}
                                >
            ...
            {loading ? (
                ...
                <div className="space-y-6">
                    {filteredRequests.map((req) => {
                        ...
                        const productImg = Array.isArray(req?.orderItem?.product?.productImages)
                            ...
                            >
                                <div className="bg-gradient-to-r from-gray-50 to-blue-50 px-6 py-4 border-b border-gray-100">
                                    <div className="flex justify-between items-center">
                                        <div className="flex items-center gap-4">
                                            <div className="font-bold text-lg text-gray-900">#{idShort}</div>
                                            <div className="flex items-center gap-2 text-gray-600">
                                                <FiCalendar size={16} />
                                                <span className="text-sm">{formatDate(req?.requestedAt)}</span>
                                            </div>
                                        </div>
                                        <span
                                            className={`px-4 py-2 rounded-full text-sm font-medium ${statusColorMap[req?.status] || "bg-gray-100 text-gray-800"
                                                }`}
                                        >
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <p className="text-sm text-gray-500">
                                                    Giá: {formatCurrency(req?.orderItem?.unitPrice)} • Số lượng:{" "}
                                                    {req?.orderItem?.quantity ?? 0}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                        ...
                        <div className="mt-6 flex justify-end gap-3">
                            ...
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg text-white ${selectedRejectReason
